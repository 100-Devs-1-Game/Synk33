...
func set_current_song(to:Song) -> void:
    ...
    var charts:Array[Chart] = to.GetAvalableCharts()
    if charts.is_empty():
    ...
    for difficulty in len(Difficulty):
        ...
        difficulty_select.set_tab_availability(
            difficulty, 
            to.HasChartInDifficulty(difficulty)
        )
    change_chart(to.GetChartByDifficulty(current_difficulty));
...
func filter_songs() -> void:
    ...
    for i in len(songs): 
        ...
        if not song.HasChartInDifficulty(current_difficulty):
            if not song.HasChart():
...
func _on_difficulty_select_tab_changed(tab: int) -> void:
    ...
    if not current_song:
        return
    if current_song.HasChartInDifficulty(tab as Difficulty):
...
func _on_play_pressed() -> void:
    if not current_song.HasChartInDifficulty(current_difficulty):
