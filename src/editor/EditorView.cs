...
public static class EditorView {
    public static void PanView(EditorState state, InputEventMouseMotion mouseMotionEvent, Action queueRedraw) {
        var deltaY = mouseMotionEvent.Position.Y - state.DragStartMouse.Y;
        state.PanY = state.DragStartPan + deltaY;
        queueRedraw();
    }

    public static void TogglePanView(EditorState state, InputEventMouseButton mouseEvent) {
    ...
    public static void ZoomAtMouse(EditorState state, float newZoom, Vector2 mousePosition, Action queueRedraw) {
